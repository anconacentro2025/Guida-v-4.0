...
// Versione motore: v6 — bump solo quando si modifica la logica in questo file, indipendente
// dalla versione generale della guida.
    ...
    const APP_CACHE_NAME = 'ancona-guida-v6.12-14080230';
...
;
    ...
    function openLightbox(photosCsv){
        ...
        photos.forEach((filename,i) => {
            slidesHtml += '<div class="gallery-slide"><img class="detail-photo loaded" src="'+PHOTO_BASE+filename+'" alt="Foto '+(i+1)+'" id="lb-img-'+i+'"></div>';
        });
        const dotsHtml = photos.length>1 ? ('<div class="gallery-dots" id="lb-dots">'+photos.map((_,i)=>'<span class="dot'+(i===0?' active':'')+'"></span>').join('')+'</div>') : '';
        const overlay = document.createElement('div');
        overlay.className = 'lightbox-overlay';
        overlay.innerHTML = '<button class="lightbox-close" aria-label="'+tr('Chiudi','Close','Schließen','Zamknij')+'">✕</button><div class="detail-gallery" id="lb-gallery">'+slidesHtml+'</div>'+dotsHtml;
        document.body.appendChild(overlay);
        overlay.querySelector('.lightbox-close').addEventListener('click', closeLightbox);
        overlay.addEventListener('click', function(e){ if (e.target === overlay) closeLightbox(); });
        if (photos.length > 1) {
            const galleryEl = overlay.querySelector('#lb-gallery'), dotsEl = overlay.querySelector('#lb-dots');
