...
async function checkAndClearCacheIfVersionChanged() {
    try {
        ...
        const newVersion = versionMeta ? versionMeta.getAttribute('content') : null;
        const savedVersion = localStorage.getItem('app-version');
        
        if (newVersion && newVersion !== savedVersion) {
            // Versione diversa — svuota tutti i cache e salva la nuova versione
            const cacheNames = await caches.keys();
            await Promise.all(cacheNames.map(name => caches.delete(name)));
            localStorage.setItem('app-version', newVersion);
            console.log('Cache cleared: version changed from', savedVersion, 'to', newVersion);
            // Force reload per gli utenti già collegati
